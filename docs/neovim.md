...
- `<leader><leader>` (Telescope buffers) is the primary way to jump between open files — `sort_mru = true` orders the list by most-recently-used rather than buffer-number order, so the handful of files actually in play right now float to the top regardless of how many stale buffers have piled up; `sort_lastused = true` additionally pre-selects the last-used entry, so `<leader><leader><CR>` with no typing toggles straight back to the previous buffer (like `<C-^>`, but through the same fuzzy-searchable list). `bufferline.nvim`'s VS Code-style tab strip (numbered tabs, `<leader>1-9` jump-by-position) and, before that, `harpoon` (a manually-curated `<leader>1-7` mark list) were both tried and removed — both are *positional* navigation (remember which number means which file), which turned out to be the actual friction: tabs auto-accumulate over a day of feature-hopping with nothing to stop them, and Harpoon's numbers were never memorable enough to actually reach for. MRU + fuzzy asks you to remember nothing — recency handles the common case, typing a few characters handles the rest.
