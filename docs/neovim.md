...
- `relativenumber = true` (stock kickstart leaves this commented out) — combined with `number`, the current line shows its real absolute number, every other line shows its distance from the cursor instead. Keeps operator+motion counts (`d5j`, `y3k`, `>4j`) small regardless of how deep into the file you are, since the number shown is always relative to right here, not to line 1.
...
- Telescope: `path_display = { 'filename_first' }` (filename first, dimmed directory second, so a long path never buries the filename — see the `mini.statusline` bullet below for the same idea there) and `initial_mode = 'normal'` (every picker opens in Normal mode instead of Telescope's default Insert mode — press `i` when you actually want to type a query). Both set in `defaults`, so they apply to every builtin picker and theme (find_files, live_grep, the `ui-select` dropdown, etc), since none of them override `initial_mode` on their own. `layout_config.horizontal.prompt_position = 'top'` puts the input field above the results list instead of Telescope's own default (below it) — scoped specifically to the `horizontal` strategy (the one actually in use here, since nothing overrides `layout_strategy`), rather than the top-level `layout_config.prompt_position`, so it doesn't affect `vertical`/`center`/other strategies' own defaults if a picker ever uses one of those instead. `sorting_strategy = 'ascending'` goes with it — Telescope's own default (`'descending'`, best match at the *bottom* of the results) made sense paired with a bottom prompt; with the prompt now on top, the best match sits at the top too, right next to it, so everything reads top-to-bottom in one direction.
- `lua/custom/plugins/nvim-notify.lua`: `nvim-notify` — replaces `vim.notify()` (used by Neovim itself and every plugin's own notify calls, including this config's own buffer auto-reload notice, save notice below, and the ISML highlighter's error notify) with animated floating toast popups instead of a plain one-line message. No overlap with `fidget.nvim`: its `override_vim_notify` option defaults to `false`, so fidget only ever renders LSP `$/progress` in its own corner widget and never touches `vim.notify()`.
- Save confirmation on `BufWritePost` (`vim.notify('Saved <filename>', ...)`) gets the same animated-toast treatment as everything else — Neovim's own default `"file" NNL, NNB written` message is a plain `:echo`, not a `vim.notify()` call, so it never picked up nvim-notify's styling on its own. `shortmess+=W` (see `:help shm-W`) suppresses that native message so only the styled one shows, rather than stacking two.
...
- `lua/custom/plugins/flash.lua`: `flash.nvim` — `<leader>j` labels every visible match for 1-2 typed characters so you can jump straight to it, further-reaching than `f`/`t` (not limited to the current line). Bound to `<leader>j` instead of flash's own default `s`/`S`: `mini.surround` already claims bare `s` outright (mapped to `<Nop>` to skip a `timeoutlen` delay on every `sa`/`sd`/`sr` press), so reusing `s` here would mean one plugin's keymap flat-out overrides the other rather than the two gracefully coexisting.
- `mini.files` (from the already-installed `mini.nvim`, so no new plugin): `<leader>e` opens a floating file explorer at the current file's directory, with that file pre-selected — falls back to cwd from an unnamed/non-file buffer. It's an editable buffer: rename a line to rename the file, `dd` to delete, add a new line with a name (append `/` for a directory, e.g. `newdir/nested/file.js` for nested paths) to create one. Press `=` to synchronize — *not* `:w`, these are special `nofile` buffers so `:w` just errors (`E382`) — which shows a confirmation dialog listing every pending action before applying anything. Replaces netrw's `:Ex`/`:Explore` as the everyday file browser (netrw itself is untouched, still there if you want it).
...
- `lua/custom/plugins/toggleterm.lua`: `toggleterm.nvim` — `<leader>tt` toggles a floating terminal for one-off shell commands (`git pull`, running a dev server, etc) without leaving Neovim. It's persistent: toggling it away and back keeps scrollback and any running process. Prefix with a count for additional independent terminals (`2<leader>tt`) — each keeps its own buffer/scrollback/process, though with a floating layout only one is ever visible on screen at once (switching to another just hides the previous one's window, it doesn't kill it). `<leader>gg` opens `lazygit` in its own dedicated toggleterm instance, kept separate from the numbered terminals so it never collides with whatever else is running — no dedicated `lazygit.nvim` plugin needed, just reusing toggleterm. Requires the `lazygit` formula (in `install.sh`'s `brew_formulae`). lazygit's own `config.yml` (`gui.showRandomTip: false`, to hide its rotating "Donate"/"Ask Question" links) is tracked and symlinked in — see [setup.md](setup.md).
- Unsaved changes are hard to miss: `mini.statusline`'s `[+]` flag is recolored (bold, matches `DiagnosticError`) instead of blending into the filename's color. (`lua/custom/winbar.lua` used to also show a bold "● unsaved" marker here, but that became redundant once `bufferline.nvim`'s tabs started showing their own modified dot right where you'd look anyway — removed.)
- `lua/custom/plugins/navic.lua`: `nvim-navic` — LSP breadcrumbs (e.g. `Basket > calculateTotal`) for the cursor's current context, shown in the winbar (`lua/custom/winbar.lua`). Works fully for any language whose LSP supports `documentSymbol` (JS/TS via `ts_ls`, Lua via `lua_ls`, etc); for `.isml` files it only gets element-nesting breadcrumbs from the `html` LSP (no function names, since ISML/HTML has none), and doesn't see into `<isscript>`/`${...}` at all — same blind spot as the base html grammar in [sfcc.md](sfcc.md).
- The winbar is styled as powerline segments (solid color pill + arrow) matching tmux's status bar and `mini.statusline`'s own styling, instead of plain text, so all three read as one continuous color scheme. (tmux's status bar itself moved to `status-position bottom` once `bufferline.nvim`'s tab strip took over the top of the editor — see [tmux.md](tmux.md) — so the winbar isn't literally adjacent to it anymore, but keeps the same styling regardless.) Colors resolve at runtime from the active colorscheme (`Normal`, `MiniStatuslineModeNormal`, `DiagnosticError`) rather than being hardcoded — blue in particular is read from `MiniStatuslineModeNormal`'s bg (tokyonight's own blue) instead of a second hardcoded copy of the tmux theme's color, specifically so this and the statusline/tmux blue can't silently drift apart again the way an earlier hardcoded copy here did.
...
- The statusline's segments are joined with powerline arrows (U+E0B0/U+E0B2, same blue used in the winbar above and tmux's status bar below — see [tmux.md](tmux.md)) instead of colors changing abruptly: right-pointing between the left-aligned segments (mode → git/diff → filename), left-pointing between the right-aligned ones (filetype → location), each cluster capped with an arrow into/out of the plain `StatusLine` background. Colors are resolved once (and on `ColorScheme`) rather than per-redraw, since only the mode segment's color actually varies frame to frame.
- `cmdheight = 0`: Neovim's default (1) permanently reserves a blank row below the statusline for typed commands/messages — with tmux's status bar sitting directly beneath (see [tmux.md](tmux.md)), that showed up as a visible empty strip wedged between the two bars. `0` reclaims that row entirely; commands/messages appear as a temporary overlay instead, only when actually needed.
...
- `lua/custom/plugins/bufferline.lua`: `bufferline.nvim` — a VS Code-style tab strip across the top of the editor, one tab per open buffer. `Shift+H`/`Shift+L` cycle prev/next tab, `<leader>t1`-`<leader>t9` jump straight to a tab by position, `<leader>bd` closes the current one. Deliberately `<leader>t1-t9`, not the more obvious `<leader>1-9` — Harpoon's own `<leader>1-7` jump keys (see the Harpoon section below) silently won that exact key sequence (identical mapping, not a prefix/timeoutlen situation), so only `<leader>8`/`9` ever actually worked as tab-jumps until this was caught and moved. Complements rather than replaces the `<leader><leader>` Telescope buffer picker below — tabs are for quick visual switching/closing among a handful of open files, Telescope stays the better tool once there are too many buffers open to scan the tab strip. Needs file-type icons (`nvim-web-devicons`, already installed as a Telescope dependency) — `vim.g.have_nerd_font` was flipped from `false` to `true` in `init.lua` for this, since this setup's terminal font already renders private-use glyphs fine elsewhere (the powerline arrows above, gitsigns' solid bar) — it was a stale leftover default, not a real constraint. Left on bufferline's own default colors/separator style — a custom trapezoid/arrow look and improved inactive-tab contrast were both attempted, but every variant showed some kind of rendering glitch in this terminal/font that couldn't be fully resolved, so reverted rather than keep guessing. `tab_size = 24` (bufferline's default, 18, packed short filenames in tightly with barely any breathing room). An `indicator = { style = 'underline' }` active-tab marker was tried and removed — didn't add anything the existing color distinction wasn't already doing. Tab groups (clustering tabs by file pattern — ISML templates, tests, config files, matched via each buffer's `name`/`path`) were also tried and removed — didn't end up adding real value in practice.
