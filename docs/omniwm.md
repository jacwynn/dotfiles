...
**App-to-app switching is Raycast**, not named per-app workspaces — `Option-C/X/E/T/N` and their `Option-Shift-` move-to-workspace counterparts (`switchWorkspaceSlot.1-5`/`moveToWorkspaceSlot.1-5`) are all deliberately `Unassigned` in `settings.toml`, along with `Control-Option-T/F/R/M`. OmniWM claiming a global hotkey blocks any other app (Raycast included) from also binding it, so these are freed up specifically for Raycast shortcuts instead. `toggleColumnTabbed`, `expandContainerToAvailablePrimarySpan`, `resetWindowSecondarySpan`, and `openMenuAnywhere` (the actions that used to live on the `Control-Option-T/F/R/M` keys) aren't bound to anything else right now. `[[appRules]]` entries auto-routing Ghostty/Chrome/Outlook to named workspaces (mirroring what AeroSpace used to do — see [aerospace.md](aerospace.md)) were also removed for the same reason; their unrelated `minHeight`/`minWidth` min-size constraints were kept.
