...
This required a `;; extends` modeline as the file's first line. Without it, Neovim treats a query file found later on `'runtimepath'` as a full *replacement* for the base query of the same name, not an addition — so the file silently discarded every other html highlight rule (headings, attributes, tag delimiters, etc) instead of layering on top of them. `;; extends` tells Neovim to merge it in instead — but the check is a strict, verbatim match on that entire first line; nothing else may share it. The initial commit put an explanatory comment on the *same* line as `;; extends` ("`;; extends -- required so...`"), which silently broke the match and reintroduced the original bug — highlighting looked fine in an isolated headless test (buffer created and attached well into the session, after everything had settled) but failed on a real `.isml` file opened normally, where the html highlighter attaches and caches its compiled query much earlier during startup. Fixed by moving the explanation to its own comment line below the bare `;; extends`, and re-verified against that exact real-file-open path (not just the isolated buffer case) before trusting it again.

### Embedded `${...}` script expressions

ISML's embedded script expressions (`condition="${pdict.foo}"`, or `${cond ? a : b}` used directly as page text) need more than the tag-name recolor above. `nvim-treesitter`'s bundled html grammar has exactly one relevant rule — inherited from lit-html support — that injects JavaScript into an attribute value, but only under the assumption that the *entire* value is one `${...}` block. That's true for lit-html's own conventions but not for ISML, where `${...}` routinely appears:

- mixed with literal text in the same attribute (`class="enroll-message ${isMemberLinked ? 'linked' : ''}"`) — the built-in rule's fixed offset then parses garbage starting partway into "enroll-message", and even where tree-sitter's error recovery accidentally salvages a token, it typically lands on `@variable`, which in `tokyonight-night` is just the plain foreground color — so it visually looks unhighlighted even though something technically matched.
- directly as page text with no attribute at all (`${isMemberLinked ? Resource.msg(...) : Resource.msg(...)}` sitting inside an `<h2>`) — the built-in rule only ever fires on attribute values, so this case gets zero JavaScript injection.

One grammar node can only carry one injected range, so neither case is fixable by adjusting that one query rule. `lua/custom/isml-expr-highlight.lua` instead scans `.isml` buffer text directly for every `${...}` run (brace-balanced, so nested `{}` in object literals don't break it), parses each one as real JavaScript via `vim.treesitter.get_string_parser`, and places the resulting highlight captures as extmarks translated back to buffer coordinates — independent of where in the grammar tree the expression happens to sit. Runs on `BufEnter`/`TextChanged`/`InsertLeave` for `*.isml` files specifically (not the broader `html` filetype), so it doesn't touch real lit-html usage elsewhere.

Caught a genuine off-by-one bug in the brace-balancer while verifying this against a real mixed-attribute example: the depth counter started at 0 instead of 1, so it could only find a "matching" close brace via re-entrant nesting, never on the simple non-nested case — silently producing zero highlights for the exact common case it needed to handle. Fixed and re-verified against both the mixed-attribute and standalone-text-expression cases before shipping.
